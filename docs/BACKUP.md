...
backup は任意機能。設定しなくても reknotes 本体・migrate は動作する (`BACKUP_S3_*` は `bun run dump` / `bun run restore` 実行時にのみ要求される)。ローカルで試すときは `docker compose -f compose.local.yaml --profile backup up` で backup 用 MinIO を起動する。
...
`bun run dump` / `bun run restore` は `loadBackupConfig()` を使う。これは primary 用の env (DATABASE_URL / S3_*) に加えて、下表の backup 固有変数を要求する。embedding を呼ばないので `CLOUDFLARE_*` は不要。
...
なし。restore も dump と同じ `loadBackupConfig()` の値を使う (= `DATABASE_URL` が復旧先 DB、`S3_*` が復旧先 bucket = 新 primary、`BACKUP_S3_*` が読み出し元 backup)。災害時は先に GitHub Secrets を新 infra に向けて deploy workflow を回し、その時点で VM 上の `.env` が新値になっているので、`bun run restore --date <YYYY-MM-DD>` を打つだけでよい。詳細は[restore (本番障害時)](#restore-本番障害時)。
...
`compose.local.yaml` は primary 用 (`minio`) と backup 用 (`minio-backup`) の 2 つの MinIO を定義する。本番の "primary と backup は別環境" の構造をローカルでも再現するため。`minio-backup` は `backup` profile に入れてあるので、起動には `--profile backup` を付ける (既定の `up` では立ち上がらない)。
...
1. `docker compose -f compose.local.yaml --profile backup up -d` で primary / backup の MinIO と Postgres を起動。
...
なお `bun run migrate --apply` の `ensureDatabaseExists()` は `DEPLOYMENT=remote` 時は no-op で抜けるよう実装されている ([postgres-migration-provider.ts:23-24](../src/app/infrastructure/providers/postgres-migration-provider.ts))。これは「managed Postgres では DB の出自はサービス側の provisioning が握っていて、アプリ側が `CREATE DATABASE` を発行する権限も妥当性も無い」という前提のため。ローカルでだけ「`POSTGRES_DB` を渡していないので postgres image が default で `reknotes` DB を作ってしまう。アプリが欲しいのは `reknotes_development` なのでアプリ側で作る」という補正が走る、という非対称になっている。
