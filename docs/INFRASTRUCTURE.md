...
外部サービスを利用し、VM (Docker が動く任意の Linux ホスト) 上で Docker Compose によって起動する。下表の「例」は動作確認済みの一例で、要件を満たせば置き換え可能。

| 役割 | 要件 | 例 |
|---|---|---|
| DB | PostgreSQL | Neon |
| Object Storage | S3 互換 API | Cloudflare R2 |
| Embedding | `infrastructure/container.ts` の remote 用 embedding provider に対応するもの (現状は Cloudflare Workers AI) | Cloudflare Workers AI |
| Reverse Proxy | TLS 終端 + 後段への proxy | Caddy |
| 認証 | 特定ユーザーのみを通せる認証 proxy | oauth2-proxy + GitHub OAuth App |
...
- **GitHub Secrets から取得**: シークレット全般 (DB 接続文字列、ストレージの API キー、Cloudflare トークン、OAuth secret、Cookie secret など) と、デプロイ先を指す `VM_HOST` / `VM_SSH_KEY`、公開ドメインを指す `DOMAIN`。
