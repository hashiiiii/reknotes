...
リモートでの公開ドメインは環境ごとに分かれる。

| 環境 | 公開ドメイン |
|---|---|
| `production` | `reknotes.hashiiiii.com` |
| `development` | 推測しづらい固定ランダムサブドメイン (`<hash>.hashiiiii.com`) |

development を秘匿サブドメインに置くのは「URL バー直打ちで偶然到達されないようにする」程度の軽い隔離が目的で、真のセキュリティ境界は oauth2-proxy + 許可 GitHub ユーザーのホワイトリスト。Let's Encrypt が発行する証明書は Certificate Transparency log に記録されるため、サブドメイン名そのものは第三者から discover 可能であることに注意。
