...
本ドキュメントは `production` 環境 1 台の構築手順をまとめたもの。設計の背景は `docs/INFRASTRUCTURE.md` を参照。
...
- Always Free 枠で AMD shape (`VM.Standard.E2.1.Micro`) が 1 台完全無料で立てられる。
...
oauth2-proxy が GitHub OAuth で認証するために、この環境用の OAuth App を 1 つ作る。
...
`CLOUDFLARE_ACCOUNT_ID` と `CLOUDFLARE_API_TOKEN` は repository-level の `Settings > Secrets and variables > Actions > Repository secrets` に登録する (`docs/INFRASTRUCTURE.md` 参照)。

### 13. 初回 deploy を流す

リポジトリの `Actions > Deploy > Run workflow` から実行する。runner が image を build → GHCR に push → Neon にマイグレーション → VM に SCP で `.env` を送る → SSH で `docker compose up -d` を流す、という順に動く。
