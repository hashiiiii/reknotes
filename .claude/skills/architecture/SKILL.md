...
このプロジェクトでは **presentation 層は二つの顔を持つ**：

- HTTP のルートハンドラ (`src/app/presentation/` 配下)
- CLI スクリプト (`scripts/` 配下)

どちらもエントリーポイントであり、application 層の use case を経由しなければならない。`domain/` や `infrastructure/` を直接呼び出してビジネスロジックを実行してはいけない。
...
関連するルールが明確になったら、実装タスクを進めてよい。自明でない設計判断を説明するときは、`docs/ARCHITECTURE.md` の該当ルールを引用すること。
